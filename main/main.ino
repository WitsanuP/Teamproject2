...
const char *topic = "teamproject/HiJame";
...
void setup() {
 ...
 client.publish(topic, "Hi Bro, Connected! ");
...
void loop() {// Wait a few seconds between measurements.
  ...
  dtostrf(h,3,0,n);
  dtostrf(t,3,0,l);
    dtostrf(hic,3,0,hicc);
	sprintf(text,"{\"temp\":\"%s\",\"hum\":\"%s\",\"hic\":\"%s\"}",l,n,hicc);
  //sprintf(text,"%.2f,%.2f,%.2f,%.2f,%.2f",h,t,f,hic,hif);
